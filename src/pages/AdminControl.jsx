...
import { 
  ...
  RotateCcw, StopCircle, ArrowRight, Clock, ExternalLink, Download, X, AlertCircle, Activity, Radio, Search, Star
} from 'lucide-react';
...
const PROJECTS = [
  { id: 1, name: "Stack Demo", points: 10 },
  { id: 2, name: "Queue Sim", points: 10 },
  { id: 3, name: "Sorting Viz", points: 10 },
  { id: 4, name: "Linked List", points: 10 },
  { id: 5, name: "Portfolio", points: 20, isBonus: true } // Bonus Project
];
      ...
      <div className="max-w-[1600px] mx-auto p-6 space-y-8 relative z-10">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-6 pt-2">
            ...
            <div className="flex gap-3">
                 ...
                 <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-red-400 font-bold text-xs transition-colors"><LogOut size={14}/> LOGOUT</button>
            </div>
        </div>

        {/* METRICS */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex flex-col justify-between h-28"><span className="text-xs font-bold text-gray-500 uppercase">Participants</span><span className="text-4xl font-black text-white">{allPlayers.length}</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-6">
                <div className="bg-[#0A0A0A] border border-white/10 rounded-3xl p-6 space-y-6">
                    ...
                    {gameState.status === 'idle' || gameState.status === 'revealed' || gameState.status === 'closed' ? (
                        <div className="space-y-4">
                            <div><label className="text-xs text-gray-400 font-bold ml-1 mb-1 block">Duration (Minutes)</label><input type="number" value={customTime} onChange={(e) => setCustomTime(Number(e.target.value))} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white font-mono text-lg outline-none" /></div>
                ...
                <div className="p-6 rounded-3xl border border-white/10 bg-[#0A0A0A] flex items-center justify-between"><div><h3 className="text-sm font-bold text-white mb-1">Leaderboard</h3><p className="text-xs text-gray-500">Show final results</p></div><button onClick={handleRevealLeaderboard} disabled={gameState.status === 'revealed'} className="px-6 py-3 rounded-xl font-bold text-xs bg-purple-600 text-white hover:bg-purple-500 shadow-lg disabled:opacity-50">REVEAL</button></div>
                <div className={clsx("p-6 rounded-3xl border transition-all", safetyLocked ? "bg-[#0A0A0A] border-white/5 opacity-60" : "bg-red-950/10 border-red-900")}><div className="flex items-center justify-between mb-4"><span className="text-xs font-bold text-red-500 uppercase tracking-widest flex items-center gap-2"><ShieldAlert size={14} /> Danger Zone</span><button onClick={() => { setSafetyLocked(!safetyLocked); setConfirmReset(false); }} className="text-[10px] font-bold border border-white/10 px-2 py-1 rounded text-gray-400 hover:text-white">{safetyLocked ? "UNLOCK" : "LOCK"}</button></div><button onClick={handleResetDB} disabled={safetyLocked} className="w-full py-3 bg-transparent border border-red-900/30 text-red-500 hover:bg-red-900/10 rounded-xl font-bold text-xs disabled:cursor-not-allowed">{confirmReset ? "CONFIRM WIPE?" : "RESET DATABASE"}</button></div>
            </div>
            ...
            <div className="lg:col-span-8 flex flex-col gap-4 h-[800px]">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4">
                    ...
                    <input type="text" placeholder="Search Participant..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-transparent text-white font-bold outline-none w-full placeholder:text-gray-600" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto custom-scrollbar pr-2 content-start flex-1">
                   {displayedPlayers.length === 0 && <div className="col-span-full text-center py-20 text-gray-600 opacity-50 font-mono uppercase tracking-widest">No participants found</div>}
                   <AnimatePresence>
                   {displayedPlayers.map(player => {
                      const hasPending = player.pending && player.pending.length > 0;
                      const isFinished = (player.projectsCompleted || 0) >= 5;
                      return (
                        <motion.div key={player.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} onClick={() => setInspectingUser(player)} className={clsx("relative p-5 rounded-2xl border cursor-pointer transition-all group flex flex-col justify-between min-h-[140px]", hasPending ? "bg-purple-900/20 border-purple-500/50 hover:border-purple-400 shadow-lg" : isFinished ? "bg-green-900/10 border-green-500/30 hover:bg-green-900/20" : "bg-[#0A0A0A] border-white/10 hover:border-white/30 hover:bg-white/5")}>
                           <div className="flex justify-between items-start mb-3">
                              <div className="flex items-center gap-4">
                                  <div className={clsx("w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg border", hasPending ? "bg-purple-500 text-white border-purple-400" : "bg-white/10 text-gray-400 border-white/5")}>{player.name.charAt(0)}</div>
                                  <div><div className="font-bold text-white text-lg leading-none mb-1">{player.name}</div><div className="text-xs text-gray-500 font-mono">XP: {player.score}</div></div>
                              </div>
                              {hasPending && <div className="px-2 py-1 rounded-md bg-purple-500 text-white text-[10px] font-bold animate-pulse shadow-lg">{player.pending.length} PENDING</div>}
                           </div>
                           <div className="mt-auto">
                               <div className="flex gap-1 mb-2">
                                  {[1,2,3,4,5].map(i => {
                                     const isDone = (player.completedProjects || []).includes(i);
                                     return <div key={i} className={clsx("h-1.5 flex-1 rounded-full", isDone ? "bg-green-500" : "bg-white/10")} />
                                  })}
                               </div>
                               <div className="flex justify-between items-center text-[10px] font-bold text-gray-500 uppercase tracking-wider"><span>{player.projectsCompleted || 0}/5 DONE</span><span className="group-hover:text-white transition-colors flex items-center gap-1">INSPECT <ArrowRight size={10}/></span></div>
                           </div>
                        </motion.div>
                      );
                   })}
        ...
        {inspectingUser && (
           <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm" onClick={() => setInspectingUser(null)}>
             <motion.div className="w-full max-w-3xl bg-[#111] border border-white/20 rounded-3xl p-8 shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="mb-8 flex items-center gap-4"><div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-3xl font-bold text-white">{inspectingUser.name.charAt(0)}</div><div><h2 className="text-3xl font-black text-white tracking-tight">{inspectingUser.name}</h2><div className="text-sm text-gray-400 font-mono">Total Score: <span className="text-purple-400 font-bold">{inspectingUser.score}</span></div></div></div>
                <div className="grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                  {PROJECTS.map((p) => {
                      ...
                      const partialPoints = Math.floor(p.points / 2);
                      const fullPoints = p.points;
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                  <div className="flex items-center gap-2">
                                    ...
                                    {p.isBonus && <span className="px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-400 text-[9px] font-bold border border-yellow-500/30 flex items-center gap-1"><Star size={8} fill="currentColor"/> BONUS</span>}
                                  </div>
                                  {isPending && inspectingUser.submissionUrl && <a href={inspectingUser.submissionUrl.startsWith('http') ? inspectingUser.submissionUrl : `https://${inspectingUser.submissionUrl}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs mt-1 hover:underline"><ExternalLink size={10} /> View Link</a>}
                              </div>
                            </div>
                            {isPending ? (
                              <div className="flex items-center gap-2">
                                  <button onClick={() => gradeUser(inspectingUser.id, p.id, partialPoints)} className="px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold text-gray-300">+{partialPoints}</button>
                                  <button onClick={() => gradeUser(inspectingUser.id, p.id, fullPoints)} className="px-4 py-2 rounded-lg bg-green-600 hover:bg-green-500 text-white text-xs font-bold flex items-center gap-1"><CheckCircle size={14} /> Accept (+{fullPoints})</button>
                              </div>
                            ) : isDone ? <div className="px-3 py-1 rounded bg-green-500/10 text-green-500 text-xs font-bold flex items-center gap-1"><CheckCircle size={12} /> Done</div> : <span className="text-xs text-gray-600 font-bold">{p.points} PTS</span>}
                        </div>
                      );
                  })}
                </div>
