...
import { 
  Send, Lock, User, Terminal, CheckCircle2, Clock, ChevronRight, 
  Link, X, Loader2, AlertCircle, PlayCircle, Radio, Hourglass, Info, FileText, AlertTriangle, Star
} from 'lucide-react';
...
const PROJECTS = [
  { 
    id: 1, 
    name: "Stack Demo", 
    desc: "Visual Stack Operations",
    details: "Create a visual demonstration of Stack operations (Push, Pop, Peek). Display the stack growing and shrinking vertically or horizontally.",
    criteria: ["Visual Push/Pop Animation", "Overflow/Underflow Handling", "Peek Current Top"]
  },
  { 
    id: 2, 
    name: "Queue Sim", 
    desc: "Live Queue Display",
    details: "Build a queue simulator visually demonstrating Enqueue and Dequeue operations. Show pointers for Front and Rear moving dynamically.",
    criteria: ["Enqueue/Dequeue Visuals", "Front/Rear Indicators", "Empty/Full State Logic"]
  },
  { 
    id: 3, 
    name: "Sorting Viz", 
    desc: "Basic Sorting Algo",
    details: "Implement a basic sorting algorithm (Bubble, Selection, or Insertion). Use bars of different heights to represent values and animate the swaps.",
    criteria: ["Random Array Generation", "Swap Animations", "Sorted State Indication"]
  },
  { 
    id: 4, 
    name: "Linked List", 
    desc: "Insert/Delete Tool",
    details: "Create a tool to visualize Singly Linked List operations. Allow users to add nodes to the head/tail and delete specific nodes.",
    criteria: ["Node Creation UI", "Pointer Connections", "Traversal Animation"]
  },
  { 
    id: 5, 
    name: "Portfolio", 
...
const StudentDeck = () => {
  ...
  const confirmSubmission = async () => {
    if (!currentUser || !selectedProject || timeLeft === 0) {
        alert("Session Ended or Invalid Request.");
        return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-24 sm:pb-10 overflow-y-auto">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 sm:gap-8">
             <div className={clsx("flex items-center gap-2 sm:gap-3 px-3 py-1.5 sm:px-5 sm:py-2 rounded-full border transition-colors", isTimeUp ? "bg-red-500/20 border-red-500" : timeLeft < 60 ? "bg-red-500/10 border-red-500/30" : "bg-white/5 border-white/10")}>
                ...
                <span className={clsx("text-lg sm:text-2xl font-mono font-bold", (timeLeft < 60 || isTimeUp) ? "text-red-400" : "text-white")}>{isTimeUp ? "00:00" : `${mins}:${secs}`}</span>
             </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 space-y-6 sm:space-y-8">
         {isTimeUp && <div className="bg-red-500/10 border border-red-500/50 p-6 rounded-3xl flex items-center justify-center gap-4 animate-pulse"><AlertTriangle size={32} className="text-red-500" /><div className="text-center"><h2 className="text-2xl font-black text-red-500 uppercase">Session Expired</h2><p className="text-red-300 text-sm">Transmissions are now locked.</p></div></div>}
                 ...
                 <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className={clsx("p-5 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all relative overflow-hidden group flex flex-col h-44 sm:h-52 justify-between", isDone ? "bg-green-900/10 border-green-500/20" : isPending ? "bg-yellow-500/5 border-yellow-500/20" : isLocked ? "bg-[#0A0A0A] border-white/5 opacity-50 grayscale" : "bg-[#0A0A0A] border-white/10 hover:border-purple-500/40 hover:bg-white/5")}>
                    ...
                    <div>
                       <div className="flex items-center gap-2">
                          ...
                          {project.isBonus && <span className="px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-400 text-[10px] font-bold border border-yellow-500/30 flex items-center gap-1"><Star size={10} fill="currentColor"/> 20 PTS</span>}
                       </div>
      ...
      <AnimatePresence>
        {infoModalProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-sm" onClick={() => setInfoModalProject(null)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full sm:max-w-xl bg-[#111] border-t sm:border border-white/20 rounded-t-[2rem] sm:rounded-[2rem] p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
